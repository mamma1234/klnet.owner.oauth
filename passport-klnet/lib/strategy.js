...
var util = require('util'),
	...
	KlnetAPIError = require('./errors/klnetapierror');

const OAUTH_HOST = 'http://localhost:5002'
// const USER_PROFILE_URL = 'http://localhost:5002/v1/nid/me'
const USER_PROFILE_URL = 'http://localhost:5002/oauth2/userinfo'
...
function Strategy(options, verify) {
	...
	console.log('0. klnet Strategy', 'options:',options, ',verify:', verify);

	options = options || {};

	// options.authorizationURL = options.authorizationURL || 'https://nid.naver.com/oauth2.0/authorize';
	options.authorizationURL = options.authorizationURL || `${OAUTH_HOST}/oauth/authorize`;
	// options.tokenURL = options.tokenURL || 'https://nid.naver.com/oauth2.0/token';
	options.tokenURL = options.tokenURL || `${OAUTH_HOST}/oauth/token`;
	...
	console.log('1. klnet Strategy', this.__options);

	OAuth2Strategy.call(this, options, verify);
	this.name = 'klnet';

	// this._profileURL = options.profileURL || 'https://openapi.naver.com/v1/nid/me';
	this._profileURL = options.profileURL || USER_PROFILE_URL;
...
 
	...
	this._oauth2.get(this._profileURL, accessToken, function (err, body, res) {
		...
		if( resultcode != "00" ){ 
			return done(new KlnetAPIError(resultmessage, resultcode));
		}
		

		var profile = { provider: 'klnet' };
